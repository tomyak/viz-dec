...
The default stdio MCP and CLI adapters connect to `adapters/shared.py`. Discovery needs no model load. The first visual request starts one detached, lazy `EngineWorker` for the installation, shared through an owner-only Unix socket. A lifetime `flock` is inherited across process startup before model construction, so concurrent requests, stale sockets, and crashed processes cannot produce duplicate model owners. A private runtime directory under `/tmp` hashes the resolved installation path to stay within macOS socket path limits. JSON requests/responses are bounded at 1 MiB/64 MiB; connections and the inference queue are bounded. No pickle is accepted.
